/**
 * Mani Library seat floor — **single-file copy** for Expo / React Native apps.
 *
 * Requires: `react`, `react-native`, `@expo/vector-icons` (FontAwesome + MaterialCommunityIcons).
 *
 * Usage:
 *   import { ManiLibrarySeatMap, type ManiLibrarySeatMapProps } from './LibrarySeatMap.standalone';
 *   <ManiLibrarySeatMap occupiedSeatIds={new Set()} selectedSeatId={n} onSelectSeat={setN} interactive />;
 *   // Optional: blockedSeatIds={new Set()} to allow every seat except occupied.
 */
...
const LEFT_SEAT_BLOCKS: SeatBlockSpec[] = [
  {
    id: 'tl',
...
const RIGHT_SEAT_BLOCKS: SeatBlockSpec[] = [
  {
    id: 'tr',
...
export type ManiLibrarySeatMapProps = {
...
export function ManiLibrarySeatMap({
  occupiedSeatIds = new Set(),
  blockedSeatIds = DEFAULT_BLOCKED_SEAT_IDS,
  selectedSeatId = null,
  onSelectSeat,
  interactive = true,
}: ManiLibrarySeatMapProps) {
  const { scheme, c } = useSeatPickerTheme();
          ...
          <View style={styles.middleBand}>
            ...
            <View style={[styles.floor, { backgroundColor: FLOOR }]}>
              <View style={styles.floorInner}>
                ...
                {/* Western edge of stairs matches bottom-left desks’ east side (same x as door jamb). */}
                {doorMarkerPos ? (
                  <View
                    pointerEvents="none"
...
const styles = StyleSheet.create({
  ...
  /** Past the floor inner edge so desks stay flush (same pattern as earlier window strip). */
  windowStripeAbs: {
    position: 'absolute',
    top: 6,
    bottom: 38,
